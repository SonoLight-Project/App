...
export default defineEventHandler(async (event) => {
    const JobId = logger.createJob(`API OAuth/GitHub/Bind #${random(9999)}`);
    try {
        logger.info("开始处理 GitHub 绑定请求", JobId);
            ...
            switch (error.statusCode) {
                case 400:
                    ...
                    throw createError({
                        ...
                        data: { errorCode: "GH_OAUTH_BIND:TOKEN_EXPIRED" },
                    });
                default:
                    ...
                    throw createError({
                        ...
                        data: { errorCode: "GH_OAUTH_BIND:UNEXCEPTED_ERROR", errorDetails: error.message },
                    });
            }
        }
        ...
        if (existingUser) {
            logger.warning("该 GitHub 账户已被绑定", JobId);
            throw createError({
                statusCode: 409,
                message: "该 GitHub 账户已被绑定",
                data: { errorCode: "GH_OAUTH_BIND:ALREADY_BOUND" },
            });
        }
        ...
        const { data, error } = await supabase
            .from("oauth")
            .update({ githubId: id, githubUsername: (userInfo as any).login })
            .eq("id", user.id)
            .select()
            .single();
        ...
        logger.info("GitHub 绑定请求处理完成", JobId);
        return {
            message: "绑定成功",
