...
export default defineEventHandler(async (event) => {
    const { code, action } = await readBody(event);

    if (!code) {
        throw createError({ statusCode: 400, message: "缺少授权码" });
    }

    // 交换code获取access token (使用application/x-www-form-urlencoded)
    let tokenResponse;
    try {
    ...
    if (action === "login") {
        try {
            ...
            throw createError({ statusCode: 500, message: "登录失败：发生意外错误，请联系管理员" });
        }
    }
    ...
    if (action === "bind") {
        try {
            ...
            throw createError({ statusCode: 500, message: "绑定失败：发生意外错误，请联系管理员" });
        }
    }
    ...
    if (action === "unbind") {
        try {
            ...
            throw createError({ statusCode: 500, message: "解绑失败：发生意外错误，请联系管理员" });
        }
    }
