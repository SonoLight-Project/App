<script lang="ts" setup>
    ...
    const in_refresh_operation = ref<boolean>(false);

    const handleRefreshUserInfo = async () => {
        if (in_refresh_operation.value) return;
        in_refresh_operation.value = true;
        try {
            ...
            setTimeout(() => (in_refresh_operation.value = false), 5000);
        }
    };
        ...
        <section id="grid-container" class="w-full grid gap-4">
            <div class="flex flex-col gap-4">
                <SonoCard class="overflow-hidden" title="用户信息">
                    ...
                    <template #extra>
                        <Icon
                            class="ml-auto -translate-y-0.5 text-error cursor-pointer hover:opacity-90 active:-translate-y-0.25"
                            :class="{ 'opacity-50! cursor-not-allowed!': in_refresh_operation }"
            ...
            <div class="flex flex-col gap-4">
                <div class="flex gap-2">
                    <label class="input w-full border-none rounded-md">
                        ...
                        <input ref="search" :placeholder="`搜索${itemTypeMapper[lastQueriedForm.item_type]}`" class="grow" type="search" />
